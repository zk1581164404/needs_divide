...
plt.rc('font', size = 18)
plt.title('Positioning Training and validation loss')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.xticks(epochs)
...
plt.title('Positioning Training and validation accuracy')
plt.xlabel('Epochs')
plt.ylabel('accuracy')
plt.legend()
plt.xticks(epochs)
...
plt.plot(np.argmax(p, axis=1) + 1,color='red', label='prediction')
...
plt.ylabel('Positioning Needs Grading Results')
...
def plot_confusion_matrix(cm, classes,title='Confusion matrix',cmap='Blues'):
    ...
    plt.tight_layout()
    plt.ylabel('预测定位需求等级',size=15)
    # plt.xlabel('true')
    plt.xlabel('实际定位需求等级\nLoss={:0.4f};Accuracy={:0.4f};Misclass={:0.4f}'.format(loss_and_metrics[0],accuracy, misclass),size=15)
    plt.savefig('Confusion_Matrix_pos.png', dpi=200, bbox_inches='tight', transparent=False)
    plt.show()
